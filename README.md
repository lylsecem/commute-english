...
- 每天打开网页点“开始今天的任务”：新词先看一遍、听发音，再按键拼写；复习的词直接听音拼写
- 拼错或用了提示的词会在最后再考一遍，全部拼对自动打卡
- 复习间隔按记忆规律安排：拼对后 1、2、4、7、15、30 天再复习；拼错就从第二天重新开始
- 点“添加每日提醒到手机日历”，每天到点手机日历会提醒
- **进度保存在当前浏览器里**，换手机、换浏览器或清除浏览器数据会从头开始
...
1. 打开仓库的 `words` 文件夹，点 **Add file → Create new file**
2. 文件名就是词书名，开头的数字用来排序，比如 `06 办公室.txt`
3. 每行一个词，格式和课程一样：
...
4. 提交后，“单词打卡”的词书下拉框里就会出现“办公室”。

也可以直接改已有的 5 本词书（日常基础、吃喝购物、出行生活、上网必备、常用动词）。
同一个单词在多本词书里只算一个，学习进度共用。
...
| `words/*.txt` | 单词打卡的词书 | **加单词改这里** |
| `audio/<节目名>/` | 自己上传的音频 | **加音频放这里** |
| `channels.json` | 网上的播客 / 直播电台 | 加网络广播改这里 |
| `courses.js` | 内置的 9 个场景、8 周路线 | 想改内置内容时改 |
| `playlist.js`、`custom_courses.js`、`words.js` | 自动生成 | 不要手改 |
| `index.html`、`vocab.js`、`update_playlist.py` | 页面和脚本 | 不用改 |
